...
/// ไดอะล็อกแก้ไขและจัดทำแผนฉุกเฉินระดับองค์กร (ERP Editor Dialog)
/// รองรับ ๖ เสาหลักตามกฎกระทรวงอัคคีภัย ๒๕๕๕ ข้อ ๔
class ErpPlanEditorDialog extends ConsumerStatefulWidget {
  final EmergencyPlanModel? existingPlan;
  final int initialStep;

  const ErpPlanEditorDialog({
    super.key,
    this.existingPlan,
    this.initialStep = 0,
  });
  ...
  @override
  ConsumerState<ErpPlanEditorDialog> createState() => _ErpPlanEditorDialogState();
}

class _ErpPlanEditorDialogState extends ConsumerState<ErpPlanEditorDialog> {
  late int _currentStep;
  late EmergencyPlanModel _editingPlan;
  bool _isSaving = false;
  bool _isRegularShift = true; // สำหรับสลับทีมดับเพลิง กะปกติ / นอกเวลา

  // Controllers สำหรับข้อมูลทั่วไป
  ...
  void initState() {
    super.initState();
    _currentStep = widget.initialStep.clamp(0, 6);
  ...
  void _loadPreset(HazardType hazard, BusinessType bType) {
    setState(() {
      ...
      _editingPlan = preset.copyWith(id: _editingPlan.id, status: _editingPlan.status);
  ...
  void _syncCurrentFormValues() {
    _editingPlan = _editingPlan.copyWith(
      ...
      fireCommanderName: _commanderCtrl.text.trim(),
      deputyCommanderName: _deputyCommanderCtrl.text.trim(),
      commanderPhone: _phoneCtrl.text.trim(),
  ...
  Future<void> _savePlan() async {
    ...
    try {
      final wasExisting = _editingPlan.id != null;
      final id = await ref.read(emergencyPlanListProvider.notifier).savePlan(_editingPlan);
      _editingPlan = _editingPlan.copyWith(id: id);
      ref.read(selectedErpPlanProvider.notifier).selectPlan(_editingPlan);
      if (mounted) {
        Navigator.pop(context, true);
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(wasExisting
                ? 'บันทึกการแก้ไขแผน "${_editingPlan.planTitle}" เรียบร้อยแล้ว (ID: #$id)'
                : 'บันทึกแผนฉุกเฉิน "${_editingPlan.planTitle}" เรียบร้อยแล้ว (ID: #$id)'),
            backgroundColor: const Color(0xFF059669),
          ),
        );
      }
  ...
  // ──────────────────────────────────────────────────────────────────────────
  // SUB-PLAN POPUP DIALOGS (Inspection, Training, Campaign, Suppression, Evacuation, Relief)
  // ──────────────────────────────────────────────────────────────────────────
  ...
  Future<void> _showInspectionItemDialog({InspectionItem? existingItem, int? index}) async {
    ...
    if (confirmed == true) {
      ...
      setState(() {
        ...
        _editingPlan = _editingPlan.copyWith(
          inspectionPlan: _editingPlan.inspectionPlan.copyWith(items: currentItems),
        );
      });
    }
  ...
  Future<void> _showTrainingCourseDialog({TrainingCourseItem? existingCourse, int? index}) async {
    ...
    if (confirmed == true) {
      ...
      setState(() {
        ...
        _editingPlan = _editingPlan.copyWith(
          trainingPlan: _editingPlan.trainingPlan.copyWith(courses: currentCourses),
        );
      });
    }
  ...
  Future<void> _showCampaignActivityDialog() async {
    ...
    if (confirmed == true && actCtrl.text.trim().isNotEmpty) {
      setState(() {
        ...
        _editingPlan = _editingPlan.copyWith(
          campaignPlan: _editingPlan.campaignPlan.copyWith(activities: currentActs),
        );
      });
    }
  ...
  Future<void> _showSuppressionRoleDialog({required bool isRegular, EmergencyTeamRole? existingRole, int? index}) async {
    ...
    if (confirmed == true) {
      ...
      setState(() {
        ...
        _editingPlan = _editingPlan.copyWith(
          suppressionPlan: _editingPlan.suppressionPlan.copyWith(
  ...
  Future<void> _showGovernmentContactDialog({EmergencyContactAgency? existingContact, int? index}) async {
    ...
    if (confirmed == true) {
      ...
      setState(() {
        ...
        _editingPlan = _editingPlan.copyWith(
          reliefPlan: _editingPlan.reliefPlan.copyWith(governmentContacts: current),
        );
      });
    }
  ...
  // ──────────────────────────────────────────────────────────────────────────
  // BUILD METHOD (Desktop Friendly Dialog Layout)
  // ──────────────────────────────────────────────────────────────────────────
  ...
  Widget build(BuildContext context) {
    ...
    final steps = [
      {'title': 'ข้อมูลทั่วไป', 'subtitle': 'สถานประกอบกิจการ', 'icon': Icons.business_outlined},
      {'title': '๑. ตรวจตรา', 'subtitle': 'จุดเสี่ยง/อุปกรณ์', 'icon': Icons.search_outlined},
      {'title': '๒. อบรม', 'subtitle': 'หลักสูตร/โควตา', 'icon': Icons.school_outlined},
      {'title': '๓. รณรงค์', 'subtitle': '5ส/ป้องกันไฟ', 'icon': Icons.campaign_outlined},
      {'title': '๔. ดับเพลิง', 'subtitle': 'ทีมระงับเหตุ', 'icon': Icons.fire_hydrant_alt_outlined},
      {'title': '๕. อพยพหนีไฟ', 'subtitle': 'จุดรวมพล/ทีมนำทาง', 'icon': Icons.directions_run_outlined},
      {'title': '๖. บรรเทาทุกข์', 'subtitle': '199/กู้ชีพ/BCP', 'icon': Icons.health_and_safety_outlined},
    ];
      ...
      child: ConstrainedBox(
        ...
        child: Column(
          children: [
              ...
              child: Row(
                children: [
                  Container(
                    ...
                    decoration: BoxDecoration(
                      color: _editingPlan.hazardType.color.withValues(alpha: 0.12),
                      borderRadius: BorderRadius.circular(10),
                    ),
                    child: Icon(_editingPlan.hazardType.icon, color: _editingPlan.hazardType.color, size: 24),
                  ),
                  const SizedBox(width: 12),
                  Expanded(
                    child: Column(
                      ...
                      children: [
                        Row(
                          children: [
                            Text(
                              _editingPlan.id != null ? 'แก้ไขแผนเผชิญเหตุฉุกเฉิน (ID: #${_editingPlan.id})' : 'สร้างแผนเผชิญเหตุฉุกเฉินใหม่',
                            ...
                            Container(
                              ...
                              decoration: BoxDecoration(
                                color: _editingPlan.hazardType.color.withValues(alpha: 0.1),
                                borderRadius: BorderRadius.circular(6),
                              ),
                              child: Text(
                                _editingPlan.hazardType.titleTh,
                                style: TextStyle(fontSize: 11, fontWeight: FontWeight.bold, color: _editingPlan.hazardType.color),
                              ),
                            ),
                          ],
                        ),
                        ...
                        Text(
                          'จัดทำและปรับปรุงข้อมูลแผนฉุกเฉิน ๖ เสาหลักตามกฎกระทรวงอัคคีภัย ๒๕๕๕ ข้อ ๔',
                  ...
                  PopupMenuButton<BusinessType>(
                    tooltip: 'โหลดแม่แบบตามประเภทธุรกิจ',
                    child: Container(
                      ...
                      child: const Row(
                        ...
                        children: [
                          ...
                          SizedBox(width: 6),
                          Text('โหลดแม่แบบมาตรฐาน', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w500)),
                          Icon(Icons.arrow_drop_down, size: 18),
                        ],
                      ),
                    ),
                    itemBuilder: (ctx) => BusinessType.values.map((b) {
                      return PopupMenuItem(
                        value: b,
                        child: Text('${b.label} (${_editingPlan.hazardType.shortTitle})'),
                      );
                    }).toList(),
                    onSelected: (b) => _loadPreset(_editingPlan.hazardType, b),
                  ),
                  const SizedBox(width: 12),
                  ...
                  Container(
                    ...
                    decoration: BoxDecoration(
                      color: _editingPlan.status.color.withValues(alpha: 0.1),
                      borderRadius: BorderRadius.circular(8),
                      border: Border.all(color: _editingPlan.status.color.withValues(alpha: 0.3)),
                    ),
                    child: DropdownButton<PlanStatus>(
                      value: _editingPlan.status,
                      underline: const SizedBox(),
                      isDense: true,
                      items: PlanStatus.values.map((s) {
                        return DropdownMenuItem(
                          value: s,
                          child: Text(s.label, style: TextStyle(fontSize: 12, fontWeight: FontWeight.bold, color: s.color)),
                        );
                      }).toList(),
                      onChanged: (s) {
                        if (s != null) setState(() => _editingPlan = _editingPlan.copyWith(status: s));
                      },
                    ),
                  ),
                  const SizedBox(width: 8),
                    ...
                    return Padding(
                      ...
                      child: InkWell(
                        ...
                        child: AnimatedContainer(
                          ...
                          padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 8),
                          decoration: BoxDecoration(
                            color: isSelected ? _editingPlan.hazardType.color : Colors.white,
                            borderRadius: BorderRadius.circular(8),
                            border: Border.all(
                              color: isSelected ? _editingPlan.hazardType.color : Colors.grey.shade300,
                            ),
                          ),
                          child: Row(
                            mainAxisSize: MainAxisSize.min,
            ...
            // ── 3. Step Content (Scrollable Form) ──
            Expanded(
              child: SingleChildScrollView(
                ...
                children: [
                  ...
                  if (_currentStep > 0)
                    OutlinedButton.icon(
                      ...
                      label: const Text('ขั้นตอนก่อนหน้า'),
                  ...
                  ElevatedButton.icon(
                    ...
                    label: Text(_editingPlan.id != null ? 'บันทึกการแก้ไข' : 'บันทึกแผนฉุกเฉิน'),
                    style: ElevatedButton.styleFrom(
                      backgroundColor: const Color(0xFF059669),
                      foregroundColor: Colors.white,
                      padding: const EdgeInsets.symmetric(horizontal: 20, vertical: 12),
  ...
  Widget _buildCurrentStepContent() {
    switch (_currentStep) {
      ...
      case 2:
        return _buildStep3Training();
      case 3:
        return _buildStep4Campaign();
      case 4:
        return _buildStep5Suppression();
      case 5:
        return _buildStep6Evacuation();
      case 6:
        return _buildStep7Relief();
      default:
        return _buildStep1General();
    }
  }
  ...
  Widget _buildStep1General() {
    return Column(
      ...
      children: [
        const Text(
          'ข้อมูลพื้นฐานสถานประกอบกิจการและผู้บัญชาการเหตุการณ์',
          style: TextStyle(fontSize: 15, fontWeight: FontWeight.bold, color: Color(0xFF0F172A)),
        ),
        const SizedBox(height: 4),
        Text(
          'ข้อมูลนี้จะปรากฏในหน้าปกและส่วนบทนำของเล่มแผนฉุกเฉินตามมาตรฐาน สปร.๔',
    ...
    return Column(
      ...
      children: [
                  ...
                  IconButton(
                    ...
                    onPressed: () {
                      setState(() {
                        ...
                        _editingPlan = _editingPlan.copyWith(
                          inspectionPlan: _editingPlan.inspectionPlan.copyWith(items: newItems),
                        );
                      });
                    },
                    tooltip: 'ลบ',
        ...
        TextFormField(
          ...
          onChanged: (val) {
            _editingPlan = _editingPlan.copyWith(
              inspectionPlan: _editingPlan.inspectionPlan.copyWith(frequencyDescription: val),
            );
          },
        ),
        const SizedBox(height: 12),
        TextFormField(
          ...
          onChanged: (val) {
            _editingPlan = _editingPlan.copyWith(
              inspectionPlan: _editingPlan.inspectionPlan.copyWith(reportingProcedure: val),
            );
          },
        ),
      ],
          ...
          children: [
            Expanded(
              child: TextFormField(
                ...
                onChanged: (val) {
                  ...
                  _editingPlan = _editingPlan.copyWith(
                    trainingPlan: _editingPlan.trainingPlan.copyWith(basicFireQuotaPercent: pct),
                  );
                },
              ),
            ),
            ...
            Expanded(
              child: TextFormField(
                ...
                onChanged: (val) {
                  _editingPlan = _editingPlan.copyWith(
                    trainingPlan: _editingPlan.trainingPlan.copyWith(annualDrillTargetMonth: val),
                  );
                },
              ),
            ),
                      ...
                      IconButton(
                        ...
                        onPressed: () {
                          setState(() {
                            ...
                            _editingPlan = _editingPlan.copyWith(
                              trainingPlan: _editingPlan.trainingPlan.copyWith(courses: current),
                            );
                          });
                        },
                        tooltip: 'ลบ',
    ...
    return Column(
      ...
      children: [
                ...
                IconButton(
                  ...
                  onPressed: () {
                    setState(() {
                      ...
                      _editingPlan = _editingPlan.copyWith(
                        campaignPlan: _editingPlan.campaignPlan.copyWith(activities: updated),
                      );
                    });
                  },
                ),
        ...
        TextFormField(
          ...
          onChanged: (val) {
            _editingPlan = _editingPlan.copyWith(
              campaignPlan: _editingPlan.campaignPlan.copyWith(smokingControlPolicy: val),
            );
          },
        ),
        const SizedBox(height: 12),
        TextFormField(
          ...
          onChanged: (val) {
            _editingPlan = _editingPlan.copyWith(
              campaignPlan: _editingPlan.campaignPlan.copyWith(hotWorkSafetyReminder: val),
            );
          },
        ),
      ],
    ...
    return Column(
      ...
      children: [
        TextFormField(
          ...
          onChanged: (val) {
            _editingPlan = _editingPlan.copyWith(
              suppressionPlan: _editingPlan.suppressionPlan.copyWith(initialResponseProtocol: val),
            );
          },
        ),
        const SizedBox(height: 12),
        TextFormField(
          ...
          onChanged: (val) {
            _editingPlan = _editingPlan.copyWith(
              suppressionPlan: _editingPlan.suppressionPlan.copyWith(majorEmergencyProtocol: val),
            );
          },
        ),
        const SizedBox(height: 16),
  ...
  Widget _buildStep6Evacuation() {
    ...
    return Column(
      ...
      children: [
        ...
        // ── SECTION: EVACUATION TEAMS ──
        Row(
          mainAxisAlignment: MainAxisAlignment.spaceBetween,
          children: [
            Text('ทีมอพยพและผู้นำทางหนีไฟ (${teams.length} ทีม):', style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 13)),
            ElevatedButton.icon(
        ...
        if (teams.isEmpty)
          Container(
            ...
            alignment: Alignment.center,
            decoration: BoxDecoration(color: Colors.grey.shade50, borderRadius: BorderRadius.circular(8)),
            child: const Text('ยังไม่ได้กำหนดทีมผู้นำทาง กดปุ่ม "+ เพิ่มทีมอพยพ"', style: TextStyle(color: Colors.grey)),
          )
        else
        ...
        if (points.isEmpty)
          ...
          ...points.asMap().entries.map((entry) {
            ...
            return Container(
              ...
              child: Row(
                children: [
                  const Icon(Icons.flag, size: 18, color: Color(0xFF059669)),
                  const SizedBox(width: 8),
                  Expanded(
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        ...
                        Text('ที่ตั้ง: ${p.location}  |  แผนก: ${p.assignedDepartments}  |  ความจุ: ${p.capacity} คน', style: TextStyle(fontSize: 10, color: Colors.grey.shade600)),
                      ],
                    ),
                  ),
                  IconButton(
    ...
    return Column(
      ...
      children: [
        ...
        TextFormField(
          ...
          onChanged: (val) {
            _editingPlan = _editingPlan.copyWith(
              reliefPlan: _editingPlan.reliefPlan.copyWith(searchAndRescueProtocol: val),
            );
          },
        ),
        const SizedBox(height: 12),
        TextFormField(
          ...
          onChanged: (val) {
            _editingPlan = _editingPlan.copyWith(
              reliefPlan: _editingPlan.reliefPlan.copyWith(damageAssessmentProtocol: val),
            );
          },
        ),
        const SizedBox(height: 12),
        TextFormField(
          ...
          onChanged: (val) {
            _editingPlan = _editingPlan.copyWith(
              reliefPlan: _editingPlan.reliefPlan.copyWith(businessContinuityProtocol: val),
            );
          },
        ),
      ],
